                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="list_id" name="list_id" required>
                                    ...
                                    <?php foreach ($activeLists as $list) { ?>
                                        <option value="<?php echo $list['id']; ?>" <?php if ($list['id'] == $selectedListId) {
                                            echo 'selected';
                                        } ?>>
                        ...
                        <tbody>
                            <?php foreach ($recentBroadcasts as $broadcast) { ?>
                                <tr>
                                    ...
                                    <td><?php $messagePreview = htmlspecialchars($broadcast['message']);
                                echo strlen($messagePreview) > 50 ? substr($messagePreview, 0, 50).'...' : $messagePreview; ?></td>
                                    <td>
                                        <?php
                                    $statusClass = '';
                                $statusText = '';
                                switch ($broadcast['status']) {
                                    case 'pending': $statusClass = 'bg-secondary';
                                        ...
                                        break;
                                    case 'in_progress': $statusClass = 'bg-warning';
                                        $statusText = 'En Progreso';
                                        break;
                                    case 'completed': $statusClass = 'bg-success';
                                        $statusText = 'Completada';
                                        break;
                                    case 'failed': $statusClass = 'bg-danger';
                                        $statusText = 'Fallida';
                                        break;
                                }
                                ?>
                                    ...
                                    <td>
                                        <?php if ($broadcast['total_contacts'] > 0) { ?>
                                            <?php $sent = $broadcast['sent_successfully'] + $broadcast['sent_failed'];
                                            $percentage = round(($sent / $broadcast['total_contacts']) * 100); ?>
